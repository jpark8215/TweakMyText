/*
  ...
  1. Schema Changes
    ...
    - Remove `credits_used` column from `rewrite_history` table
  ...
  4. Notes
    ...
    - Removes obsolete credits_used column since tokens are tracked at user level
*/
...
-- Remove obsolete credits_used column from rewrite_history table
DO $$
BEGIN
  -- Drop credits_used column if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'rewrite_history' AND column_name = 'credits_used'
  ) THEN
    -- First drop the constraint if it exists
    IF EXISTS (
      SELECT 1 FROM information_schema.table_constraints 
      WHERE constraint_name = 'rewrite_history_credits_used_check' 
      AND table_name = 'rewrite_history'
    ) THEN
      ALTER TABLE rewrite_history DROP CONSTRAINT rewrite_history_credits_used_check;
    END IF;
    
    -- Then drop the column
    ALTER TABLE rewrite_history DROP COLUMN credits_used;
  END IF;
END $$;
